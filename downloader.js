...
class ImageDownloader {
  constructor(baseUrl) {
    this.baseUrl = baseUrl; // ex: "https://nhentaibr.com"
  ...
  async download({ dataUpload, name, numberImageArr, outputDir }) {
    // Cria pasta específica do álbum
    ...
    const tasks = numberImageArr.map((n) =>
      this.limit(async () => {
        const basePath = `${dataUpload}/${name}-${n}`;
        const dest = path.join(albumDir, `${n}.jpg`); // salva dentro da pasta do álbum
